...
productsController.postProduct = async (req, res) => {
  try {
    const { name, category, price, stock, status, sku, supplier } = req.validatedBody; // obtenemos los datos
    const newProduct = new Product({ name, category, price, stock, status, sku, supplier  }); // creamos una nueva instancia del producto con los datos recibidos
